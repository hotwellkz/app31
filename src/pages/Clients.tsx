...
export const Clients: React.FC = () => {
  ...
  const handleStatusChange = async (clientId: string, newStatus: 'building' | 'deposit' | 'built') => {
    try {
      // Получаем все номера клиентов для нового статуса в текущем году
      ...
      // Генерируем новый номер для клиента
      ...
      batch.update(clientRef, {
        ...
        // Автоматически скрываем иконки если статус "построено"
        isIconsVisible: newStatus !== 'built'
      });

      // Получаем данные клиента для обновления категорий
      const clientDoc = await getDoc(clientRef);
      if (clientDoc.exists()) {
        const clientData = clientDoc.data();

        // Находим и обновляем категории
